document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleBillingToggle = (e) => {
        ...
        // --- 2. Update Content of each Card ---
        pricingCards.forEach(card => {
            const planId = card.getAttribute('data-plan-id'); // "Starter", "Pro", "Premium"
            const priceEl = card.querySelector('[data-price-point]');
            const periodEl = card.querySelector('[data-billing-period]');
            const buttonEl = card.querySelector('.get-started-btn');
            
            // --- 3. Determine the correct translation keys ---
            const priceKey = selectedPeriod === 'monthly' ? `pricing${planId}PriceMonthly` : `pricing${planId}Price`;
            const periodKey = selectedPeriod === 'monthly' ? 'monthlyPaymentBilling' : 'oneTimePayment';

            // --- 4. Apply a micro-animation for a smooth update ---
            // We add the 'updating' class, which triggers the CSS animation
            ...
            setTimeout(() => {
                ...
                const planName = `${planId}${selectedPeriod === 'monthly' ? ' Monthly' : ''}`;
                buttonEl.setAttribute('data-plan', planName);
